...
import { LayoutDashboard, ClipboardList, Activity, ArrowRight, Sparkles, Folder } from "lucide-react";

export default function WelcomePage() {
  const router = useRouter();
  const [session, setSession] = useState(null);

  // ปรับปรุงการเช็ค Session เพื่อป้องกัน Hydration error ใน Next.js
  ...
  // โครงสร้างข้อมูลการ์ดพร้อมการแต่งสีธีม Ocean
  const cards = [
    {
      href: "/dashboard",
      label: "Dashboard",
      desc: "ดูสรุปภาพรวมการผลิตและ KPI ประจำวัน",
      badge: "OVERVIEW",
      icon: Activity,
    },
    {
      href: "/form",
      label: "กรอกรายงานการผลิต",
      desc: "บันทึกข้อมูลการผลิต EMBOSS ประจำกะ",
      badge: "DAILY ENTRY",
      icon: ClipboardList,
    },
    {
      href: "/tracking",
      label: "ติดตามการผลิต",
      desc: "ตรวจสอบสถานะงานและประวัติย้อนหลัง",
      badge: "MONITORING",
      icon: Folder,
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      {/* 🌊 1. HERO SECTION: โทนสีน้ำเงินมหาสมุทรไล่ระดับ (Ocean Gradient) */}
      <div className="relative bg-linear-to-r from-[#021338] via-[#003B73] to-[#00B4D8] text-white pt-12 pb-28 px-6 md:px-12 overflow-hidden">
        {/* แสงเอฟเฟกต์วงกลมโปร่งแสงด้านหลัง (Background Bubbles) */}
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/10 border border-white/20 backdrop-blur-md text-cyan-100 text-xs font-medium mb-4 shadow-sm">
            <span>{session.department} · E-Production Report EMBOSS</span>
          </div>

          {/* ข้อความทักทาย */}
          <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight leading-tight">
            สวัสดี, <span className="text-transparent bg-clip-text bg-linear-to-r from-white via-cyan-100 to-amber-300">{session.firstName} {session.lastName}</span> !
          </h1>
          <p className="text-cyan-100/80 mt-4 text-sm md:text-base max-w-xl leading-relaxed">
            ยินดีต้อนรับเข้าสู่ระบบ เลือกเมนูการทำงานที่ต้องการได้จากรายการด้านล่าง
          </p>
        </div>
      </div>

      {/* 💳 2. CARDS SECTION: การ์ดเมดลอยเกยขึ้นมาบน Hero Header */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          {cards.map(({ href, label, desc, badge, icon: Icon }) => (
            ...
            >
              ...
              {/* ปุ่มกด Action ด้านล่างการ์ด (สีส้ม/เหลืองสดตามสไตล์ Ocean Royale) */}
